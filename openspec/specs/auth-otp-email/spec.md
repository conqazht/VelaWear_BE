# auth-otp-email Specification

## Purpose
TBD - created by archiving change add-auth-otp-email. Update Purpose after archive.
...
The system MUST send OTP emails through an internal provider abstraction with Resend/Gmail SMTP as implementations.

#### Scenario: OTP email sent through configured provider
- **WHEN** OTP generation succeeds and provider is configured
- **THEN** the system sends the OTP email through the configured provider implementation
...
### Requirement: OTP Email Request and Verification (Hardened)
The `auth-otp-email` capability MUST use `auth:otp:v2:*` challenge/proof state. The legacy verified-marker requirement is superseded and MUST NOT be accepted as proof.

#### Scenario: Submit legacy verified marker
- **WHEN** a final account action submits only OTP v1 verified-marker state without a valid `otpProofToken`
