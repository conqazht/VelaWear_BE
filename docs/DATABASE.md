...
| type | VARCHAR(50) | NOT NULL, CHECK in ('ORDER_UPDATE', 'PROMOTION', 'SYSTEM', 'PAYMENT') | Category/type |
| target_type | VARCHAR(50) | NULLABLE, CHECK target_type IS NULL OR target_type IN ('ORDER', 'PRODUCT', 'SALE_CAMPAIGN', 'COUPON') | Related entity type |
| target_id | VARCHAR(100) | NULLABLE | Identifier for related entity |
| link_url | VARCHAR(255) | NULLABLE | Navigation URL on client click |
| image_url | VARCHAR(500) | NULLABLE | Optional notification icon/thumbnail |
...
- `INDEX idx_notifications_user_unread ON notifications(user_id) WHERE is_read = FALSE`

Notes:
- `user_id` uses `ON DELETE CASCADE` so deleting an account cascades notifications.
- Partial index `idx_notifications_user_unread` optimizes customer unread badge counter queries.
...
| notifications | `idx_notifications_user_created` | `user_id`, `created_at DESC` | Paginated customer notification list |
| notifications | `idx_notifications_user_unread` | `user_id` (WHERE `is_read = FALSE`) | Fast unread badge counter |
...
| User -> Notification | OneToMany | Notification | `@JoinColumn(name = "user_id")` |
...
- Persistent `notifications` table is implemented (`V28__create_notifications.sql`) to store customer-specific notifications (order status changes, promotions, system alerts) with user indexing and partial index for fast unread badge counting; cascaded on user deletion.
